...
from mirai.core.api.chat import clear_session
from mirai.core.api.chat import generate_chat_events as _legacy_generate_chat_events
...
from mirai.core.services import ChatTurnService, EdgeService
...
# Compatibility target for tests and downstream integrations that monkeypatch
# ``mirai.core.api.routes.generate_chat_events``.
generate_chat_events = _legacy_generate_chat_events
...
app.state.runtime = _state.get_runtime()
...
async def websocket_edge_endpoint(websocket: WebSocket):
    ...
    await EdgeService(websocket.app.state.runtime).handle_peer(peer)
...
async def chat_endpoint(request: Request, identity: CurrentIdentity, body: ChatRequest):
    ...
    async def generate():
        compat_routes = sys.modules.get("mirai.core.api.routes")
        compat_generate_chat_events = getattr(compat_routes, "generate_chat_events", None)
        if compat_generate_chat_events is not None:
            stream = compat_generate_chat_events(body.prompt, sid, think=body.think)
        else:
            stream = ChatTurnService(request.app.state.runtime).stream_chat_turn(body.prompt, sid, think=body.think)
