...
into ``isinstance`` / ``match`` dispatch (see :mod:`yumi.core.platform.http.stream_consumer`).
External callers that still want plain dicts get them via ``model_dump`` at
the public boundary in :mod:`yumi.core.features.chat.pipeline`.
...
class ErrorEvent(BaseModel):
    ...
    ``YUMI_CHAT_PIPELINE_FAILED``); ``content`` is a human-friendly message.
    """
